...
df %>% filter(!is.na(accessibility)) %>% write_csv("../data/processed-old.csv")
```
...
cut2 = Hmisc::cut2
df %>% 
    mutate(ebin=cut2(evaluation, g=3)) %>% 
    filter(considered) %>% 
    chisq_test(ebin ~ control) %>% kable(digits=3)
...
df %>% 
    ...
    group_by(control) %>% 
    mutate(prop = n/sum(n)) %>% 
    ggplot(aes(ebin, n, fill=control)) + geom_bar(stat="identity", position='dodge')


q1 = df %>% with(quantile(evaluation, 1/3))
q3 = df %>% with(quantile(evaluation, 2/3))
df %>% 
    filter(considered) %>% 
    mutate(bad=evaluation < q1) %>% 
    group_by(control) %>% 
    chisq_test(bad ~ control) %>% kable(digits=3)
    

df %>% 
    filter(considered) %>% 
    mutate(good=evaluation < q1) %>% 
    chisq_test(good ~ control) %>% kable(digits=3)
```
...
df %>% 
    filter(considered) %>% 
    mutate(evaluation = evaluation) %>% 
    group_by(control,wid) %>% 
    summarise(evaluation = mean(evaluation)) %>% 
...
prop.test(low$good, low$total, 1/3, 'greater')
prop.test(low$bad, low$total, 1/3, 'greater')
prop.test(low$neutral, low$total, 1/3, 'less')

prop.test(high$good, high$total, 1/3, 'greater')
prop.test(high$bad, high$total, 1/3, 'less')
prop.test(high$neutral, high$total, 1/3, 'less')

prop.test(X$bad, X$total, alternative='greater')
...
X %>% 
    ggplot(aes(ebin, fill=control)) + geom_bar(position="dodge") + control_colors
...
X %>% 
    mutate(x=ebin=="good") %>% 
    prop_test(x ~ control, success='TRUE')
